defmodule Matplotex.Figure do
  ...
  @margin_default 0.1
  @figsize {10,6}
  defstruct [:id, :axes, :element,valid?: true,rc_params: %RcParams{}, errors: [], figsize: @figsize, rows: @row_column_default, columns: @row_column_default, margin: @margin_default]
  @type t() :: %__MODULE__{
    id: String.t(),
    axes: any(),
    element: any(),
  ...
  def add_label(%__MODULE__{axes: %module{} = axes} = figure, label, opts), do: %{figure | axes: module.add_label(axes, label, opts) }
  def add_title(%__MODULE__{axes: %module{} = axes} = figure, title,opts), do: %{figure | axes: module.add_title(axes, title,opts)}
  ...
  def set_rc_params(figure, params) when is_map(params) do
    params = if string_keys?(params) do
