    ...
    label_elements =
      Enum.map(labels, fn {axis, label} ->
        ...
        %Label{
          ...
          y: to_pixel(y),
          text: label,
          height: to_pixel(height),
          width: to_pixel(width)
        }
        ...
        %Figure{
          axes:
            %{
              ...
              size: {_width, height},
              element: elements,
              coords: %Coords{bottom_left: {blx, bly}, y_ticks: {ytx, yty}} = coords
            } = axes
        } = figure
      ) do
  ...
  def cast_hgrids(
        %Figure{
          axes:
            %{
              show_h_grid: true,
              coords: %Coords{hgrids: hgrids, top_left: {_tlx, topy}},
              element: elements
            } = axes
        } = figure
      ) do
  ...
  def cast_vgrids(
        %Figure{
          axes:
            %{
              show_v_grid: true,
              coords: %Coords{vgrids: vgrids, top_right: {rightx, _topy}},
              element: elements
            } = axes
        } = figure
      ) do
